...
// });
	...
	$('.audioPitch').on('input', function(e) {
		...
		$set= Math.round($set*10)/10;
		$(this).next().text($set);
		pitchy= e.currentTarget.value*500;
		c(pitchy)
		var audioSrc = blobURL;
...
function getImgData() {
	...
	if (file.length>0) {
	  	...
	  	fileReader.onload = event => {imgPreview.setAttribute("src", event.target.result);imgReview.setAttribute("src", event.target.result);};
